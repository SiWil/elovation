...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE USER elovation_admin WITH PASSWORD '$ELOVATION_DB_ADMIN_PASSWORD';
    CREATE USER elovation_user WITH PASSWORD '$ELOVATION_DB_USER_PASSWORD';
    ...
    GRANT USAGE ON SCHEMA public TO elovation_admin;
    GRANT USAGE ON SCHEMA public TO elovation_user;
    ALTER DATABASE elovation_db owner TO elovation_admin;
EOSQL
